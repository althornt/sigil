...
# Read in non batch corrected tables 
# PS
df_all_PS_nobc <- read.table(file = paste0(sigil_out_path,"combine_mesa_out/merged_mesa_allPS.tsv"),
                          sep="\t", header = TRUE, row.names=1) 
df_all_PS_nobc <- df_all_PS_nobc %>% mutate_if(is.character,as.numeric)

# IR
df_IR_table_nobc <- read.table(file =paste0(sigil_out_path,
                          "/combine_mesa_out/merged_mesa_ir_table_intron_retention.tsv"),
                          sep="\t", header = TRUE, row.names=1) 
df_IR_table_nobc <- df_IR_table_nobc %>% mutate_if(is.character,as.numeric)

# gene
df_exp_nobc <- read.table(file= paste0(sigil_out_path,
                    "combine_gene_out/combined_kallisto_log2tpm.csv"),
                    sep=",", header = TRUE, row.names=1)
df_exp_nobc <- df_exp_nobc %>% mutate_if(is.character,as.numeric)

print(dim(df_all_PS_nobc))                      
print(dim(df_IR_table_nobc))                      
print(dim(df_exp_nobc))                      
...
legend_umap <- get_legend(
  gene_by_group$umap + theme(legend.position="top", legend.title = element_blank(), legend.text = element_text(size = 12))
)
# Get legend from plot
legend_pca <- get_legend(
  gene_by_group$pca + theme(legend.position="top", legend.title = element_blank(), legend.text = element_text(size = 12))
)
# Add the legend to the row 
sigil_by_group_pca <- plot_grid(prow_pca_sigil, legend, ncol = 1, rel_heights = c(1, .3))
ggsave(file.path(fig_output_path,
...
sigil_by_group_umap <- plot_grid(prow_umap_sigil, legend, ncol = 1, rel_heights = c(1, .3))
ggsave(file.path(fig_output_path,
...
#   gene_by_main$pca + theme(legend.position="top")
# )
...
# Plot all by group label
prow_pca_all_genes_bc <- plot_grid(
...
# Add the legend to the row 
all_genes_juncs_by_group_pca <-plot_grid(prow_pca_all_genes_bc, legend, ncol = 1, rel_heights = c(1, .3))
ggsave(file.path(fig_output_path,
...
prow_umap_all_genes_bc <- plot_grid(
...
all_genes_juncs_by_group_umap <- plot_grid(prow_umap_all_genes_bc, legend, ncol = 1, rel_heights = c(1, .3))
ggsave(file.path(fig_output_path,
...
###################################################
# GROUP LABEL ALL JUNCTIONS/GENES BEFORE BATCH CORRECTION
# Make PCAs by group label 

# Make PCAs by group label 
gene_by_group_all_no_bc <- plotPCA(df_exp_nobc,rownames(df_exp_nobc), "Gene_sigil_by_group", "group_label", "Gene" )
PS_by_group_all_no_bc <- plotPCA(df_all_PS_nobc,rownames(df_all_PS_nobc), "PS_sigil_by_group", "group_label", "Splice" )
IR_by_group_all_no_bc <- plotPCA(df_IR_table_nobc,rownames(df_IR_table_nobc), "IR_sigil_by_group", "group_label", "Intron Retention" )

# Plot all by group label
prow_pca_all_genes_no_bc <- plot_grid(
  gene_by_group_all_no_bc$pca + theme(legend.position="none"),
  PS_by_group_all_no_bc$pca + theme(legend.position="none"),
  IR_by_group_all_no_bc$pca + theme(legend.position="none"),
  align = 'vh',
  # labels = c("A", "B", "C"),
  hjust = -1,
  nrow = 1
...
# Add the legend to the row 
all_genes_juncs_by_group_pca_no_bc <-plot_grid(prow_pca_all_genes_no_bc, legend, ncol = 1, rel_heights = c(1, .3))
ggsave(file.path(fig_output_path,
                paste0("PCA_all_by_group_all_juncs_or_genes_no_bc.png")),
        device = "png",
        width = 15, height = 6,
        dpi = 300, bg="white")

prow_umap_all_genes_no_bc <- plot_grid(
  gene_by_group_all_no_bc$umap + theme(legend.position="none"),
  PS_by_group_all_no_bc$umap + theme(legend.position="none"),
  IR_by_group_all_no_bc$umap + theme(legend.position="none"),
  align = 'vh',
  # labels = c("A", "B", "C"),
  hjust = -1,
  nrow = 1
...
all_genes_juncs_by_group_umap_no_bc <- plot_grid(prow_umap_all_genes_no_bc, legend, ncol = 1, rel_heights = c(1, .3))
ggsave(file.path(fig_output_path,
                paste0("UMAP_all_by_group_all_juncs_or_genes_no_bc.png")),
        device = "png",
        width = 15, height = 6,
        dpi = 300, bg="white")
...
# plot_grid(all_genes_juncs_by_group_pca_no_bc, all_genes_juncs_by_group_pca,sigil_by_group_pca, sigil_by_main_pca, ncol = 1)
plot_grid(legend_pca,prow_pca_all_genes_no_bc, prow_pca_all_genes_bc,prow_pca_sigil, ncol = 1, rel_heights = c( .35, 1, 1, 1))
...
# plot_grid(all_genes_juncs_by_group_umap_no_bc, all_genes_juncs_by_group_umap,sigil_by_group_umap, sigil_by_main_umap, ncol = 1)
plot_grid(legend_umap,prow_umap_all_genes_no_bc, prow_umap_all_genes_bc,prow_umap_sigil, ncol = 1, rel_heights = c( .35, 1, 1, 1))
ggsave(file.path(fig_output_path,
