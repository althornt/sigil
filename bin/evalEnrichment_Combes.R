...
Combes2set <- list(
  ...
  "myeloid" = list(list("myeloid"), myeloid_sets )
)
##########################
# Functions
##########################
...
calc <- function(df_enr, name){
  ...
  for ( i in unique(metadata$group_label)){
    ...
    if (i %in% unlist(Combes2set[[1]][1])){
      df_enr <- checktop(i, unlist(Combes2set[[1]][2]), df_enr, df_maxmin)
    } 
    # Treg
    else if (i %in% unlist(Combes2set[[2]][1])){
      df_enr <- checktop(i, unlist(Combes2set[[2]][2]), df_enr, df_maxmin)
    } 
    # Myeloid
    else if (i %in% unlist(Combes2set[[3]][1])){
      df_enr <- checktop(i, unlist(Combes2set[[3]][2]), df_enr, df_maxmin)
    }
    } 
